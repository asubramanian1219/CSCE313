...
private:
	...
	condition_variable slot_available; //waited on my push, signaled by pop
...
public:
	...
	void push(char* data, int len){
		...
		//3. Then push the vector at the end of the queue, watch out for race conditions
		q.push(d);
		l.unlock();
		//4. wake up popped threads
	...
	int pop(char* buf, int bufcap){
		...
		//2. pop the front item of the queue. The popped item is a vector<char>, watch out for race conditions
